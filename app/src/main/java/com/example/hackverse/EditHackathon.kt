...
import com.example.hackverse.databinding.EdithackathonBinding
...
class EditHackathon : AppCompatActivity() {
    private lateinit var firebaseAuth: FirebaseAuth
    private lateinit var binding: EdithackathonBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = EdithackathonBinding.inflate(layoutInflater)
        ...
        if (UID != null) {
            val userRef2= FirebaseFirestore.getInstance().collection("Hackathons").document(UID)
            userRef2.get().addOnSuccessListener { documentSnapshot ->
                if (documentSnapshot.exists()) {
                    binding.header.text = "Edit "+documentSnapshot.getString("Title")
                    binding.title.setText(documentSnapshot.getString("Title"))
                    binding.organisingauthority.setText(documentSnapshot.getString("Organising Authority"))
                    binding.organisingevent.setText(documentSnapshot.getString("Organising Event"))
                    binding.date.setText(documentSnapshot.getString("Date"))
                    binding.eligibility.setText(documentSnapshot.getString("Eligibility"))
                    binding.eligibilityd.setText(documentSnapshot.getString("Eligibility Details"))
                    binding.size.setText(documentSnapshot.getString("Size"))
                    binding.lastdate.setText(documentSnapshot.getString("Last Date"))
                    binding.venue.setText(documentSnapshot.getString("Venue"))
                    binding.overview.setText(documentSnapshot.getString("Overview"))
                    binding.structure.setText(documentSnapshot.getString("Structure"))
                    binding.judging.setText(documentSnapshot.getString("Judging"))
                    binding.reward.setText(documentSnapshot.getString("Reward"))
                    binding.email.setText(documentSnapshot.getString("Email"))
                    binding.number.setText(documentSnapshot.getString("Number"))
                    ...
                    if(documentSnapshot.getString("Fee")=="Free"||documentSnapshot.getString("Fee")=="0")
                    {
                        ...
                        binding.paid.setText(documentSnapshot.getString("Fee"))
                    }
                }
            }
        }
        ...
        binding.upload.setOnClickListener{
            ...
            if ( title.isNotEmpty() && lastdate.length>15 && fee != null && organisingevent.isNotEmpty() && organisingauthority.isNotEmpty() && venue.isNotEmpty() && date.isNotEmpty() && lastdate.isNotEmpty() && size.isNotEmpty() && eligibility.isNotEmpty() && overview.isNotEmpty() && eligibilityd.isNotEmpty() && structure.isNotEmpty() && judging.isNotEmpty() && reward.isNotEmpty() && name.isNotEmpty() && number.isNotEmpty() && email.isNotEmpty()) {
                ...
                db.collection("Hackathons").document(UID)
                    .update(hackathon as Map<String, Any>)
                    .addOnSuccessListener {
                        binding.upload.text = "Updating..."
                        binding.upload.setTextColor(Color.parseColor("#808080"))
                        binding.upload.backgroundTintList =
                        ...
                        selectedImageUri?.let { it1 ->
                            uploadImageToCloudinary(it1, contentResolver, UID) { url ->
                                if (url != null) {
                                    db.collection("Hackathons").document(UID)

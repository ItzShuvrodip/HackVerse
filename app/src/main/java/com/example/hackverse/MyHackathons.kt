...
import com.example.hackverse.databinding.MyhackathonsBinding
...
class MyHackathons : AppCompatActivity() {
    private lateinit var firebaseAuth: FirebaseAuth
    private lateinit var binding: MyhackathonsBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = MyhackathonsBinding.inflate(layoutInflater)
        ...
        binding.navHome.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                binding.colorSlider.post {
                    binding.colorSlider.x = binding.navMyhackathons.x
                    binding.colorSlider.visibility = View.VISIBLE
                    binding.navMyhackathons.animate()
                        .scaleX(1.1f)
                        .scaleY(1.1f)
                        .setDuration(200)
                }
        ...
        if(binding.participated.currentTextColor==Color.parseColor("#FFFFFF")) {
        ...
        binding.participated.setOnClickListener {
                    binding.slider.post {
                        binding.refresh.isRefreshing = true
                        binding.slider.animate()
                            .x(binding.participated.x + 14)
                            .setDuration(400)
                            .setInterpolator(AccelerateDecelerateInterpolator())
                            .withEndAction {
                                binding.created.setTextColor(Color.parseColor("#036261"))
                                binding.participated.setTextColor(Color.parseColor("#FFFFFF"))
                                getData1(search)
                                setupRecyclerView()
                            }
                            .start()
        ...
        binding.created.setOnClickListener {
            binding.slider.post {
                binding.refresh.isRefreshing = true
                binding.slider.animate()
                    .x(binding.created.x + 2)
                    .setInterpolator(AccelerateDecelerateInterpolator())
                    .withEndAction {
                        binding.participated.setTextColor(Color.parseColor("#036261"))
                        binding.created.setTextColor(Color.parseColor("#FFFFFF"))
                        getData2(search)
                        setupRecyclerView()
                    }
                    .start()
        ...
        binding.navCalendar.setOnClickListener {
            binding.colorSlider.post {
                binding.colorSlider.animate()
                    ...
                    .setInterpolator(AccelerateDecelerateInterpolator())
                    .withEndAction {
                        binding.myhackathonstext.setTextColor(Color.parseColor("#2E2E2E"))
                        binding.calendartext.setTextColor(Color.parseColor("#036261"))
                        binding.myhackathonsicon.imageTintList=ColorStateList.valueOf(Color.parseColor("#2E2E2E"))
    ...
    private fun getData1(SearchQuery: String? = null) {
        val db = Firebase.firestore
        firebaseAuth = FirebaseAuth.getInstance()
        val user = firebaseAuth.currentUser
        val userID = user?.uid
                    ...
                    for (document in result) {
                        ...
                        if (participatedHackathonUIDs.contains(uid)) {
                            if ((SearchQuery != null && ((title.contains(
                                    ...
                                    SearchQuery,
                                    ignoreCase = true
                                ))) || SearchQuery == null) && lastDateTime.after(
                                    currentDateTime
                                ) && eventDateTime.after(currentDateTime)
                            ) {
                                openEvents.add(
                                    Hackathon(
                                        uid,
                                    ...
                                    SearchQuery,
                                    ignoreCase = true
                                ))) || SearchQuery == null) && lastDateTime.before(
                                    currentDateTime
                                ) && eventDateTime.after(currentDateTime)
                            ) {
                                registrationClosedEvents.add(
                                    ...
                                    currentDateTime
                                ) && eventDateTime.before(currentDateTime)
                            ) {
                                closedEvents.add(
                                    Hackathon(
                                        uid,
    ...
    private fun getData2(SearchQuery: String? = null) {
        val db = Firebase.firestore
        firebaseAuth = FirebaseAuth.getInstance()
        val user = firebaseAuth.currentUser
        val userID = user?.uid
        ...
        db.collection("Hackathons")
            ...
            .addOnSuccessListener { result ->
                ...
                for (document in result) {
                    ...
                    val sdf = SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.getDefault())
                    val sdf2 = SimpleDateFormat("dd/MM/yyyy", Locale.getDefault())
                    val userIdentity=document.getString("UserID")?:"N/A"
                    val lastDateTime = sdf.parse(lastDate)
                    val eventDateTime = sdf2.parse(date)
                    val currentDateTime = Date()
